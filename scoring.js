...
$(document).ready(function () {
  ...
  function score_indicate() {
    ...
    // 変数「average」に平均値を出して代入しましょう(平均をとりたい数の合計点数(sum) / 全体の個数)
    // 全体の個数はlengthメソッドを使って求めます。
    // (lengthメソッド: 文字列の長さや配列の要素数などを取得するメソッド)
  ...
  function judgement() {
    ...
    // 「最終ジャッジ」(id="alert-indicate)ボタンを押したら
    //  「あなたの成績は${achievement}です。${pass_or_failure}です。」が出力される処理です。
    $('#declaration').append(`<label id="alert-indicate" class="alert alert-info">あなたの成績は${achievement}です。${pass_or_failure}です。</label>`);
  };
  // [国語の点数,英語の点数,数学の点数,理科の点数,社会の点数]のいずれかの点数が変更された際に
  // 「function score_indicate()」を発火させる処理です。
  ...
  // 「最終ジャッジ」(id="btn-declaration")ボタンが押された際、
  //  「function judgement()」の処理を実行させる。
  // ２回目以降に「最終ジャッジ」ボタンを押した際は、それまでに表示していたジャッジのHTML要素を削除して、
  //  新たなジャッジのHTML要素を追加する。
